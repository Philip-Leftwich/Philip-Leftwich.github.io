...
<div class="news-listing">
  <% for (const [index, item] of items.entries()) { %>
    <article class="news-listing__item<% if (!item.image) { %> news-listing__item--no-image<% } %>" aria-labelledby="news-item-title-<%= index %>">
      <% if (item.image) { %>
      ...
      <div class="news-listing__content">
        ...
        <% } %>
        <h3 class="news-listing__title" id="news-item-title-<%= index %>">
